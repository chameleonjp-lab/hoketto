...
現在はP3のホーム・基本説明・試合選択導線、説明1/3の照準確認、2/3の指を離す確認、3/3の充電輪待機確認、説明から開く実物理射撃場、30秒の試し撃ち、HTML結果画面、3盤面の試作、れんしゅう／ふつうCPU、両砲台の状態表示、同条件再戦、効果音・音楽の個別設定とWeb Audio合成音、音設定の端末内保存・破損時の無音復旧、試遊記録の端末内保存・コピー本文の生成、基本説明の完了状態保存、2点コアの予告・出現・得点、無得点圧力、キーボード照準・発射・停止・3秒再開、延長中の20%ゴール拡大、再開カウント中の再中断復旧、固定更新の遅延蓄積監視、試合目的と状態通知、公開用ワークフロー、勝ち方の表示、端末別操作案内、充電ゲージ、黒い盤面、弾・パックの軌跡、命中輪、反射火花、ゴール輪、得点停止後の満タン再開、CPUの危険対象優先と判断記録に加え、画面非表示・bfcache復帰・向き変更・表示領域変更・WebGL復元時のシステム中断を自動検査した段階です。P4先行のブラウザ導線スモーク検査（ホーム→試合、説明→射撃場、ホーム→試合、手動停止・明示再開、画面非表示からの明示再開、音設定の再訪復元）をPlaywrightへ追加しました。試合選択では3盤面、れんしゅう／ふつうCPU、30秒の試し撃ちまたは90秒試合を開始可能としています。射撃場はCPUと時計を止め、外しても時間切れにならず、命中後の得点で練習完了を表示します。ホーム、説明、試合選択中はPhaserを生成せず、試合または射撃場へ入った後に盤面を生成します。自動検査は通過しましたが、iPhone実機、初見試遊、CPUの勝率調整、独立レビュー、J-PlatPatの確定検索はまだ完了していません。未確認の項目を「通過」と記録しません。
...
| 射撃場の物理・再試行             | 通過 | `tests/unit/gameFlow.test.ts`・`straightBench.test.ts`、実物理への遷移、時計・CPU停止、外した後の再発射を確認                                                                                                |
| 得点停止後の満タン再開           | 通過 | `tests/unit/straightBench.test.ts`、両者の充電を0へ戻し、プレイヤーはREADY、CPUは反応待ちから再開することを確認                                                                                              |
| H10〜H12ゲームプレイ監査         | 通過 | `npm run audit:gameplay`、720試合・24条件でRESULT到達、`invalid=0`、tick上限0、READY違反0、候補・終盤イベントを記録。物理追補は [`quality/physics-audit-2026-09-12.md`](quality/physics-audit-2026-09-12.md) |
| CPU判断の再現性・危険対象優先    | 通過 | `tests/unit/straightBench.test.ts`、同じ状態の同じ照準計画、上向きの高得点パックを先に守ることを確認                                                                                                         |
...
| 射撃場の命中・得点         | 保留 | 射撃場の導線と純粋な物理は自動検査済み。iPhone Safariで実際に命中・得点できるかは未確認     |
...
| H14演出の実機見え方        | 保留 | 消音時の因果表示は自動検査済み。iPhone Safariでの軌跡、輪、火花の大きさと重なりは未確認     |
