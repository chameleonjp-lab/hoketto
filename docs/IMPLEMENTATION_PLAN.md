...
| コア予告 | 固定出現位置の六角形、数字「2」、残り2秒 | 全画面の警告色と盤面を隠す説明 |
...
入力では `pointerId` を記録し、最初の有効な指だけを追う。`pointerdown` の時点で発射可能かを固定し、`pointerup` の時点で都合よく発射可能へ変えない。指を置いた後に盤面か画面端の有効領域から外れる、`pointercancel`、得点、時間切れ、延長予告、結果、中断、画面非表示、横向き、表示矩形の変更が起きた場合は操作を破棄する。発射イベントの固定更新番号と座標は、通常試合中に有効な `pointerup` を受けた時だけ確定する。一時停止は狙い中でも優先し、結果画面へ移る時は全Pointer状態を消して、直前の指離しを「もういちど」へ渡さない。
...
入力のブラウザ試験では、短いタップ、押したまま方向変更、盤外と画面端帯での取り消し、`pointercancel`、充電中に始めた操作、2本指、狙い中の得点・時間切れ・中断・画面変形、試合から結果への指離しを別々に確認する。各試験で射撃回数と充電消費回数が一致し、意図しない後発射が0件であることを求める。キーボードは、Tabで盤面へ入り抜けでき、押し続けなくても矢印キーだけで狙い印を有効領域の端まで動かし、EnterとSpaceで発射、Escapeで停止できることを確認する。

### 6.11 デザイン資料と素材の受け渡し

P0で `ART_DIRECTION.md` の初版を作り、色の役割、文字、形、輪郭、描画順、動きを抑える場合の置き換えを記録する。`UX_FLOW.md` には、初回と再訪の画面遷移、ボタンの順番、入力の状態、説明、延長、中断、結果を記録する。P2へ進む前に、375×667と393×852の二つで、ホーム、試合、狙い中、充電中、コア予告、得点、延長、中断、横向き、結果、復元失敗の静止画をそろえる。通常表示、明度だけの表示、動きを抑える表示を確認し、承認した画像を基準として残す。
...
最初に仮称『ホケット』の名称を予備調査し、検索日、対象、結果を記録する。その後に、375×667の画面遷移と操作状態の簡単な画面案、`ART_DIRECTION.md` と `UX_FLOW.md` の初版、プロジェクトの土台、1個の弾とパック、Phaser 4の技術試作、物理の最小試作、試験と公開出力の骨組みを作る。

通過条件は、名称調査記録、初回と再訪の遷移図、発射可能、狙い中、充電中、コア予告、延長、中断、結果の画面案があること。そのうえで、iPhone相当の縦画面で座標が一致し、弾が高速でもパックを通り抜けず、同じ入力から同じ検査結果を得られ、リポジトリ名から求めた基準パスの本番出力を開けること。固定した依存関係で `npm ci`、型検査、静的検査、単体試験、本番出力が成功することも必要とする。技術選定を変える場合は、この段階で文書も更新する。

### P1：遊べる試作

ストレート・ベンチ、通常パック、両砲台、仮CPU、90秒、得点、結果、再戦を作る。絵と音は仮でよい。

通過条件は、30試合連続で進行し、直接攻撃と横からの守備がどちらも有効であること。パック消失、二重得点、時間停止の失敗を0件にする。弾の1回反射、リコシェ・レール、反射の応用説明はP3で実装し、まとめて判定する。
...
通過条件は、初見10人中8人以上が30秒以内に命中し、9人以上が初回試合を完了し、6人以上が終了後30秒以内に再戦を始め、8人以上が基本規則と充電輪の意味を説明できること。10人中8人以上が、色を消した画像でも通常パックと2点コア、通常壁と反射板、発射可能と充電中を区別できること。標準CPUは、固定した盤面と乱数の組を使う計30試合でプレイヤーが12～18勝に入ること。不達の場合は機能追加を止め、原因、変更値、参加者の重複有無、再確認結果を記録する。

### P4：公開候補

権利台帳、第三者ライセンス、容量と性能、表示、復帰、GitHub Pagesの公開処理を確認する。

通過条件は、重大不具合0件、CPU同士100試合で進行不能0件、新しいブラウザ状態で公開URLから説明、試合、結果、再戦を完走できること。iPhone SafariとWebKit自動試験の両方で確認する。375×667と393×852では、通常表示、動きを抑える表示、200%文字拡大を確認する。読み込み、ホーム、説明、選択、試合、得点、延長、中断、横向き、結果、復元失敗をChromiumとWebKitで個別に画像比較し、動く画面は固定更新番号、基準画像はブラウザ別にする。
...
| 6 | 純粋な物理の最小実装 | 弾、パック、壁、ゴールの単体試験と再現データ |
...
| 13 | P2完成見本の独立レビュー | 通過、不通過、保留を `TEST_REPORT.md` に残す |
