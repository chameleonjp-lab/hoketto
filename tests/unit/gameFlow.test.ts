...
describe('application flow', () => {
  it('起動時はホームを表示し、説明の進行位置を先頭にする', () => {
    expect(createAppFlowState()).toEqual({ screen: 'HOME', tutorialStep: 0 });
  });

  it('ホームから基本説明を先頭で開く', () => {
    expect(openTutorial(createAppFlowState())).toEqual({ screen: 'TUTORIAL', tutorialStep: 0 });
  });
  ...
  it('基本説明を最後まで進めると試合へ移る', () => {
    ...
    expect(nextTutorial(state)).toEqual({ screen: 'GAME', tutorialStep: TUTORIAL_STEP_COUNT - 1 });
  });
  ...
  it('すぐ遊ぶ、戻る、説明外の次へは安全に扱う', () => {
    ...
    expect(returnHome()).toEqual({ screen: 'HOME', tutorialStep: 0 });
  });
});
