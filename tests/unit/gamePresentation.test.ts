...
describe('game presentation', () => {
  it('端末の入力特性からスマホとPCの案内を切り替える', () => {
    expect(
      detectDeviceMode({ coarsePointer: true, finePointer: false, touchPoints: 5 }),
    ).toBe('touch');
    expect(
      detectDeviceMode({ coarsePointer: false, finePointer: true, touchPoints: 0 }),
    ).toBe('desktop');
    expect(
      detectDeviceMode({ coarsePointer: false, finePointer: false, touchPoints: 1 }),
    ).toBe('touch');
    expect(
      detectDeviceMode({ coarsePointer: false, finePointer: false, touchPoints: 0 }),
    ).toBe('desktop');
