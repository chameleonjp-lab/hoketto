...
現在はP2の基本導線試作段階です。起動時のホーム、3手順の基本説明、説明を飛ばして遊ぶ導線、縦画面のCanvas描画、固定更新の物理、Pointer入力、弾・パック・ゴール・得点・90秒時計、同じ物理規則で動くCPUの自動射撃、両砲台の発射状態表示、HTMLの結果画面、同条件再戦を実装しています。CPUの難易度調整、複数盤面、音、正式な試合選択は次の工程です。
