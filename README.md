...
現在はP1のストレート・ベンチ試作段階です。縦画面のCanvas描画、固定更新の物理、Pointer入力、弾・パック・ゴール・得点・90秒時計、同じ物理規則で動くCPUの自動射撃、両砲台の発射状態表示、結果からの同条件再戦を実装しています。CPUの難易度調整、複数盤面、音、正式なホーム・結果画面は次の工程です。
